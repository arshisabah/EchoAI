...
from app.services.meeting_room_manager import get_meeting_room_manager, ParticipantRole
...
from app.modules.audio_recorder import (
...
async def create_meeting_room(request: CreateRoomRequest):
    ...
    try:
        ...
        maybe = store.create_session(
            room_id, {"room_name": request.room_name, "created_by": request.created_by}
        )
...
async def end_meeting_room(room_id: str, ended_by: str = Query(...)):
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                f"Warning: delete_session failed (maybe missing): {e}", exc_info=True
            )
    ...
    try:
        ...
        return {
            ...
            "transcript": json.loads(
                json.dumps([e.to_dict() for e in entries], default=str)
            ),
...
async def extract_tasks(room_id: str):
    ...
    try:
        ...
        tasks = await task_engine.extract_tasks_from_transcript(
            [e.to_dict() for e in transcript_entries], room_id, participants
        )
...
async def get_meeting_summary(room_id: str):
    ...
    try:
        ...
        summary_result = await summary_service.generate_structured_summary(
            transcript_texts, room_id, mode="final"
        )
...
async def download_meeting_recording(room_id: str):
    ...
    try:
        ...
        if not recorder:
            raise HTTPException(
                status_code=404, detail="No recording found for this room"
            )
...
async def download_meeting_transcript(
    room_id: str, format: str = Query("txt", regex="^(txt|json|srt)$")
):
    """
    ...
    try:
        ...
        if not transcript_entries:
            raise HTTPException(
                status_code=404, detail="No transcript found for this room"
            )
        ...
        if format == "txt":
            ...
            raise HTTPException(
                status_code=400, detail="Invalid format. Use txt, json, or srt"
            )
...
async def meeting_websocket(
    ...
    try:
        ...
        if not existing_room:
            await room_manager.safe_send_text(
                websocket, {"type": "error", "message": f"Room {room_id} not found"}
            )
        ...
        if existing_room and user_id in existing_room.participants:
            ...
            try:
                ...
                logger.debug(
                    f"Failed to close old websocket for {user_id}", exc_info=True
                )
        ...
        actual_role = (
            ...
            if (room_for_role_check and room_for_role_check.created_by == username)
        ...
        try:
            ...
            logger.error(
                f"Failed to join room {room_id} for user {user_id}: {e}", exc_info=True
            )
            await room_manager.safe_send_text(
        ...
        await room_manager.safe_send_text(
            websocket, {"type": "peer_list", "peers": peers}
        )
        ...
        try:
            ...
            if historical_transcripts:
                logger.info(
                    f"📜 Sending {len(historical_transcripts)} historical transcripts to {username}"
                )
                ...
                for i in range(0, len(historical_transcripts), batch_size):
                    ...
                    for entry in batch:
                        ...
                        if current_room and speaker_id in current_room.participants:
                            speaker_name = current_room.participants[
                                speaker_id
                            ].username
                        ...
                        transcript_message = {
                            ...
                            "text": entry.text,
                            "emotion": (
                                entry.emotions.get("emotion", "neutral")
                                if entry.emotions
                                else "neutral"
                            ),
                            "confidence": (
                                entry.emotions.get("confidence", 0.0)
                                if entry.emotions
                                else 0.0
                            ),
                            "emotion_guidance": {},
            ...
            logger.error(
                f"Failed to send historical transcripts to {username}: {e}",
                exc_info=True,
            )
        ...
        while not stop_event.is_set():
            try:
                ...
                logger.error(
                    f"WebSocket receive error in {room_id}: {e}", exc_info=True
                )
                break
            ...
            if message_type == "audio_chunk":
                ...
                await room_manager.safe_send_text(
                    websocket, {"type": "chat_ack", "message": chat_message["message"]}
                )
            ...
            # WebRTC signaling routing: target_id must be present and will be forwarded
                ...
                logger.info(
                    f"📡 WebRTC signaling: {message_type} from {username} ({user_id}) to {target_id} in room {room_id}"
                )
                ...
                if target_id and room_id in room_manager.rooms:
                    ...
                    if target_id in room.participants:
                        ...
                        success = await room_manager.safe_send_text(
                            target_ws,
                            {
                                **message,
                                "from_id": user_id,
                        ...
                        if success:
                            logger.info(
                                f"✅ Forwarded {message_type} from {username} to {target_id}"
                            )
                        else:
                            logger.error(
                                f"❌ Failed to forward {message_type} to {target_id}"
                            )
                    else:
                        logger.warning(
                            f"⚠️ Target {target_id} not found in room {room_id}. Available participants: {list(room.participants.keys())}"
                        )
                else:
                    logger.warning(
                        f"⚠️ Signaling message without target_id or room {room_id} missing. Message: {message_type}"
                    )
                ...
                logger.debug(
                    f"Unknown WS message from {username} in {room_id}: {message_type}"
                )
        ...
        try:
            ...
            logger.debug(
                f"Error during leave_room cleanup for {user_id}", exc_info=True
            )
...
async def process_audio(room_id, user_id, username, message, room_manager, websocket):
    ...
    try:
        logger.info(
            f"🎵 process_audio called - room: {room_id}, user: {username} ({user_id})"
        )

        # 1️⃣ Extract base64 audio from message
        audio_base64 = (
            message.get("audio") or message.get("audio_data") or message.get("data", "")
        )
        if not audio_base64:
            logger.warning(
                f"❌ No audio data found for user {username} in room {room_id}"
            )
            return
        ...
        try:
            ...
            logger.info(
                f"✅ Decoded audio chunk from {username}: {len(audio_bytes)} bytes"
            )
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Failed to add audio to recorder for room {room_id}: {rec_err}"
            )

        # 3️⃣ Run unified AI pipeline via orchestrator
        logger.info(
            f"🔧 Calling orchestrator for room {room_id}, user {username} with {len(audio_bytes)} bytes"
        )
        orchestrator = get_orchestrator_service()
        result = await orchestrator.process_audio_chunk(
            audio_bytes=audio_bytes, session_id=room_id, participant_id=user_id
        )
        logger.info(
            f"✅ Orchestrator returned result for room {room_id}: {type(result).__name__ if result else 'None'}"
        )

        # 🔹 Handle lightweight "listening" heartbeats
        if result and result.get("type") == "listening":
            logger.debug(
                f"⏳ Buffering audio for {username} in {room_id}: {result.get('buffered_duration', 0):.2f}s"
            )
            try:
                await websocket.send_json(
                    result
                )  # send only to this user (not broadcast)
            except Exception as send_err:
                logger.warning(
                    f"⚠️ Failed to send listening status to {username}: {send_err}"
                )
            return
        ...
        if not result:
            logger.debug(
                f"⚠️ No result from orchestrator for user {username} in {room_id}"
            )
            return
        ...
        for entry in entries:
            ...
            speaker = (
                entry.get("speaker") or user_id
            )  # prefer orchestrator speaker, fallback to user_id
            ...
            logger.info(
                f"📢 Broadcasting transcript from {username} in {room_id}: '{text[:50]}...'"
            )
            ...
            logger.info(
                f"🗣️ [{speaker}] '{text[:60]}' | Emotion={emotion} ({confidence:.2f})"
            )
